                                ...
                                <tbody>
                                    @foreach ($bachat as $bachats)
                                        <tr>
                                            ...
                                            <td>{{ $bachats->mr }}</td>
                                            <td>{{ $bachats->mobile_no_1 }}</td>
                                            <td>{{ $bachats->village }}</td>
                                            <td>{{ $bachats->city }}</td>
                                            <td>{{ $bachats->taluk }}</td>
                                            <td class="n">
                                                @if (Auth::user()->role_as == '1')
                                                    <a href="{{ url('admin/bachat/' . $bachats->id . '/edit') }}"
                                                        class="btn btn-success">Edit</a>
                                                @else
                                                    &nbsp;
                                                @endif
                                                <a href="{{ url('admin/bachat/' . $bachats->id . '/view') }}"
                                                    class="btn btn-primary">View</a>
                                                @if (Auth::user()->role_as == '1')
                                                    <a href="{{ url('admin/bachat/' . $bachats->id . '/delete') }}"
